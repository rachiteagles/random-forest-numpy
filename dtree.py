...
class DecisionTree621:
    ...
    def fit_(self, X, y):
        ...
        col,split = self.best_split(X,y,self.loss)
    ...
    def predict(self, X_test):
        ...
        result = []
        for row in X_test:
            result.append(self.root.predict(row))
    ...
    def best_split(self, X,y,loss):
        best = {'col' : -1,'split' : -1,'loss':loss(y)}
        index = np.random.choice(len(X.T),int(round(len(X.T)*self.max_features)),replace=False)
        # print(index)
        for col in index:
            if len(X.T[col]) >= 11:
                cand = np.random.choice(list(X.T[col]),11,replace=False)
            else:
                cand = X.T[col]
            for split in cand:
...
class ClassifierTree621(DecisionTree621):
    ...
    def score(self, X_test, y_test):
        ...
        return (y_pred.flatten() == y_test).sum()/len(y_test)
